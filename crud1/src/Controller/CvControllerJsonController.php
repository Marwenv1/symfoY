...
class CvControllerJsonController extends AbstractController
{
    ...
    {
        $tasks = $this->getDoctrine()->getManager()->getRepository(Cv::class)->findAll();
    ...
    public function addAction(Request $request,NormalizerInterface $Normalizer)
    {
        ...
        $cv->setTelephone($request->get('telephone'));
        $cv->setAdresse($request->get('adresse'));
        $cv->setCodepostale($request->get('codepostale'));
        $cv->setVille($request->get('ville'));
        $cv->setDatedenaissance($request->get('datedenaissance'));
        $cv->setLieu($request->get('lieu'));
        $cv->setSexe($request->get('sexe'));
        $cv->setEtatcivil($request->get('etatcivil'));
        $cv->setFormation($request->get('formation'));
        $cv->setEtablif($request->get('etablif'));
        $cv->setStage($request->get('stage'));
        $cv->setEtablis($request->get('etablis'));
        $cv->setExperience($request->get('experience'));
        $cv->setTablie($request->get('tablie'));
        $cv->setCentredinteret($request->get('centredinteret'));
        $cv->setPhoto($request->get('photo'));
        $cv->setLongtitude($request->get('longtitude'));
        $cv->setAltitude($request->get('altitude'));
        $em->persist($cv);
        $em->flush();
